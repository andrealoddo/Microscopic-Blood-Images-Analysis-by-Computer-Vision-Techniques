...
\section{Dissertation structure} % TODO
...
\section{Colour and Colour Spaces} % DONE R
...
\subsection{Primary Spaces}  % DONE
...
% TODO aggiungere paragrafo sulla morfologia matematica
...
\section{Double Segmentation}
The first realised method for white blood cells segmentation makes use of two segmentation steps performed on two different images. In contrast to other reported methods, this approach does not require separate steps of pre-processing and segmentation; it uses pre-processing steps inserted among the various stages of segmentation to make the latter simpler and more robust. As said previously, other methods aim first to identify the nuclei, which are more prominent than other components \cite{Madhloom}, and then to detect the entire membrane (i.e., by region growing \cite{Kovalev,Cheewatanon,Lez02}). In contrast, this approach detects the membrane first, in order to perform the further step of separation of the adjacent cells more accurately.  The algorithm consists of the following phases:
\begin{itemize}
\item Conversion from RGB to CMYK colour model
\item Histogram equalisation
\item Segmentation by threshold using Zack algorithm
\item Background removal operation
\end{itemize}
As said previously, the WBCs nuclei are more in contrast in the Green component of the RGB colour space. But to segment adequately the whole leukocyte this colour space is not the best choice. In fact it has been observed that leukocytes are more contrasted in the Yellow component of CMYK colour space \cite{Put13a}, because the yellow colour is present in all elements of the image, except leukocytes (Fig.~\ref{fig:example4} shows two examples). So, the first phase consists in converting the original RGB image into the CMYK image (\ref{cmyk}). The Yellow channel presents an high contrast between leukocytes and the other image components, but at the same time it presents an histogram with intensity values too close between them. For this reason a redistribution of the intensity values is necessary to make the subsequent segmentation process easier. Then, a histogram equalisation (\ref{histop}) has been used to redistribute the intensity value equally (see Fig.~\ref{fig:example4}). The result of this operation seems to present some noise, as histogram equalisation enhances the non uniformity of the background, typical of this kind of images. At the same time histogram equalisation creates clear valley between the modes of the histogram preserving also the shape of the white blood cells. Segmentation is achieved using a threshold automatically calculated by the Zack algorithm (\ref{Zack}), since this algorithm is particularly effective when the histogram displays clear valleys between high and weak peaks. The results of the segmentation with threshold are very good as it can be seen in Fig.~\ref{fig:example4}. Then the complement image is calculated to obtain WBCs on a dark background.

\begin{figure}[!tbp]
\centering
%\includegraphics[height=0.23\textheight]{images/Fig2-1}\vspace{0.5mm}
%\includegraphics[height=0.23\textheight]{images/Fig2-2}
%\includegraphics[height=0.23\textheight]{images/Fig3-1}\vspace{0.5mm}
%\includegraphics[height=0.23\textheight]{images/Fig3-2}
%\includegraphics[height=0.23\textheight]{images/Fig4-1}\vspace{0.5mm}
%\includegraphics[height=0.23\textheight]{images/Fig4-2}
%\includegraphics[height=0.23\textheight]{images/Fig5-1}
%\includegraphics[height=0.23\textheight]{images/Fig5-2}
\caption{\label{fig:example4}Top to bottom: original blood sample images, Y component images, histogram equalisation results and segmentation results.}
\vspace{4mm}%
\end{figure}

Considering that the images captured at the microscope suffer from uneven lighting, it becomes necessary to remove the background because the segmentation methods based on threshold may suffer heavily for this problem. Some approaches for background extraction have been described in literature, but they use a collection of images captured with the same camera and the same microscope for estimation of the pixels belonging to the background \cite{Sco05}, while others have a very high computational cost, not necessary for this application. The proposed approach involves the use of an automatic threshold applied to a single image. This threshold could be applied to the grey level image or to the Green channel of the RGB colour space. The choice is made automatically by the algorithm that analyses the contrast of the two images looking for the one presenting a less condensed histogram, in order to avoid further operations of grey level redistribution. The threshold value is calculated again using the Zack algorithm (\ref{Zack}). Fig.~\ref{fig:example5} shows how the result may not be accurate in all aspects, in fact even the centre of red blood cells can be detected as background. However, this does not preclude the achievement of effective background removal because the goal now is to preserve only the WBCs present in the image. Background removal can be performed using simple arithmetical operations, by subtracting the background just extracted from the image segmented starting from the Y component. Obviously, the background removal process does not produce a clean result for the whole image. To clean up the image, the area opening operation (\ref{postp}) has been used, which allows to delete all the objects with a size smaller than the average size of the objects in the image.

\begin{figure}[!tbp]
\centering
%\includegraphics[height=0.23\textheight]{images/Fig7-1}\vspace{0.5mm}
%\includegraphics[height=0.23\textheight]{images/Fig7-2}
%\includegraphics[height=0.23\textheight]{images/Fig8-1}\vspace{0.5mm}
%\includegraphics[height=0.23\textheight]{images/Fig8-2}
%\includegraphics[height=0.23\textheight]{images/Fig10-1}
%\includegraphics[height=0.23\textheight]{images/Fig10-2}
\caption{\label{fig:example5}Top to bottom: grey level images, background identification results and background removal results.}
\end{figure}

As it can be seen in Fig.~\ref{fig:example5} the segmentation results of this approach are really effective, being able to individuate and segment all the WBCs. 

\section{Fuzzy Threshold}
Nevertheless, when the image presents noise or imprecision the segmentation process becomes complex, leading to ill defined regions. So, in this case it is appropriate to avoid a crisp threshold and to prefer a fuzzy threshold. Furthermore, images acquired from digital microscope are affected from uneven lighting, with a central area very bright, actually caused by the light of the microscope and the presence of shading area more marked towards the corners. Very few papers are designed to achieve a robust segmentation performance under uneven lighting conditions. Low pass filter have been used for background removal \cite{Sco06} in order to improve the segmentation results based on threshold operations. In \cite{Cha10} instead a local threshold based on FSs have been used in order to calculate a different threshold value for each windows avoiding problems related to uneven illumination. The obtained results are very encouraging, but they have been obtained making some arithmetical operation between the threshold value in order to obtain good segmentation results. Moreover the optimal threshold computation is particularly expensive. This approach have been realised taking inspiration from \cite{Cha10}, thus using the local fuzzy threshold to improve the segmentation accuracy. To better understand this approach a brief introduction on FS theory is needed.   

\subsection{On Fuzzy Sets}
Fuzzy Sets (FSs) theory was proposed in 1965 by Zadeh \cite{ZadI, ZadII, ZadIII} as a novel way of representing indeterminateness, in order to provide a tool able to describe the characteristics of a too complex or ill-defined system to admit precise mathematical analysis. This theory reflects also the human reasoning which is not based on traditional two-valued or even multi-valued logic, but it is based on fuzzy logic truths. In traditional set theory, if we have a set $A$ such that  $ \{0, 1\} \in A$, an element has only two options: it belongs totally to the set $A$ or it doesn't belong at all to the set $A$. This set is called crisp set. According to Zadeh the concept of set is more flexible introducing the idea of membership. So, if we have a FS $A$ in a universe $X$, and $x$ is some element in $X$, the degree of membership $\mu$ is some real number between $[0, 1]$, indicating how much $x$ belongs to $A$.
A FS $A$ in a universe $X$ is a set of ordered pairs
\begin{equation}
A=\{(x, \mu_A(x)) | x \in X\}
\end{equation}
where the function $\mu_A(x) : X \rightarrow [0,1]$ defines the degree of membership of the element $x \in X$. It is obvious that an element presents also a degree of non-membership, which is automatically set to 1 minus the degree of membership. 
Human being who expresses the degree of membership of a FS not always can be sure that the degree of non-membership of an element in a FS is equal to the complement to 1. That is to say, there may be some hesitation degree. Based on this idea, Atanassov \cite{Atan} in 1983 introduced the concept of Intuitionistic Fuzzy Sets (\acs{IFS}s) that contains both a membership and a non-membership degree. An IFS $A$ in a universe $X$ may be represented as
\begin{equation}\label{IFS}
A=\{(x, \mu_A(x), \upsilon_A(x)) | x \in X\}
\end{equation}
where the functions $\mu_A(x), \upsilon_A(x)  : X \rightarrow [0,1]$ define, respectively, the degree of membership and the degree of non-membership of the element $x \in X$, with the condition $0 \le \mu_A(x) + \upsilon_A(x) \le 1$.
Using the definition (\ref{IFS}) it can be observed that every FS is a particular case of IFS
\begin{equation*}
A=\{(x, \mu_A(x) , 1-\mu_A(x))  | x \in X\}.
\end{equation*}
Afterwards, a third parameter for the hesitation degree $\pi_A(x)$ has been proposed \cite{Szmidt}, which gives the possibility to take into account also lack of knowledge or personal error that can arise in computing distances among FSs. Using this parameter the representation of an IFS $A$ can be rewritten as
 \begin{equation}\label{AIFS}
A=\{(x, \mu_A(x), \upsilon_A(x), \pi_A(x)) | x \in X\}
\end{equation}
with the condition $\mu_A(x) + \upsilon_A(x) + \pi_A(x) = 1$. So, it is obvious that $0 \le \pi_A(x) \le 1$ for each $x \in X$.
As important contents in fuzzy mathematics, similarity measure and distance measure between IFSs have attracted many researchers. The intuitionistic fuzzy divergence (IFD) between two IFSs measures how much two sets differ between them. Many divergence formulae have been proposed in literature, such as a generalization of the distance measures from classical FS theory, Hamming distance, normalized Hamming distance, Euclidean distance and normalized Euclidean distance, by incorporating the non-membership function \cite{Atan} and then adding the hesitation degree \cite{Szmidt}. In \cite{Cha03} a fuzzy divergence measure between two FSs, $A$ and $B$ has been introduced. It is defined as the sum of the discrimination of $A$ against $B$, and the discrimination of $B$ against $A$:
\begin{align*}
D_{AB}&=D_{AB}^1 + D_{BA}^2\\
&=\sum_{x\in X}[2 - (1 - \mu_A(x) + \mu_B(x)) e^{\mu_A(x) - \mu_B(x)} \\
&-(1 - \mu_B(x) + \mu_A(x))e^{\mu_B(x) - \mu_A(x)}].
\end{align*}
In a more recent work \cite{Cha10} this divergence measure has been extended to IFS, including the hesitation degree. So, for two IFS $A$ and $B$ the divergence measure can be defined as 
\begin{equation}\label{DIV}
\begin{split}
D_{AB}&=\sum_{x\in X}[2 - (1 - \mu_A(x) + \mu_B(x))e^{\mu_A(x) - \mu_B(x)} \\
&- (1 - \mu_B(x) + \mu_A(x))e^{\mu_B(x) - \mu_A(x)}  \\
&+2 - (1 - \mu_A(x) - \pi_A(x) + \mu_B(x) + \pi_B(x)) \\
&\cdot e^{\mu_A(x) + \pi_A(x) - \mu_B(x) - \pi_B(x)}\\
&- (1 - \mu_B(x) - \pi_B(x) + \mu_A(x) + \pi_A(x)) \\
&\cdot e^{\mu_B(x) + \pi_B(x) - \mu_A(x) - \pi_A(x)}].
\end{split}
\end{equation}

\subsection{Global IFS threshold}
Many approaches for image thresholding based on fuzzy methods have been proposed and, recently, IFSs have been used to determine the optimal threshold value for grey level image segmentation \cite{Cha10, MeloP}. In fact the hesitation degree value can represent the hesitance of an expert in determining whether a pixel of the image belongs to the background or to the object of the image. Furthermore, using IFS we can transform an ill-defined segmentation problem into a precise and well-defined optimization problem, by minimizing the divergence between the actual image and the ideally thresholded image in order to find the optimal threshold. An ideally thresholded image \cite{Cha03} is an image where the background and the foreground regions are precisely segmented and all pixels present the values $\mu_B(x)=1$ and $\pi_B(x)=0$. So, the divergence measure in (\ref{DIV}) can be rewritten as
\begin{equation}\label{DIV2}
\begin{split}
D_{AB}&=\sum_{x\in X}[2 - (2 - \mu_A(x))e^{\mu_A(x) - 1} - \mu_A(x)e^{1- \mu_A(x)}  \\
&\qquad +2 - (2 - \mu_A(x) - \pi_A(x))e^{\mu_A(x) + \pi_A(x) - 1}\\
&\qquad - (\mu_A(x) + \pi_A(x)) e^{1 - \mu_A(x) - \pi_A(x)}].
\end{split}
\end{equation}
The main disadvantage of using a IFSs formulation is the relatively high computational complexity. In fact, IFSs are used to create an intuitionistic fuzzy, by computing the membership, non-membership and hesitation degree for each pixel. Then the final threshold can be obtained by minimizing the IFD between the original image and the ideally thresholded image. Therefore, the idea is to modify again the divergence function in order to directly process the grey levels, rather than pixels values, and to use the histogram values as weights. So, let $\{0,1,2,...,L-1\}$ denote the $L$ distinct intensity levels in a digital image, the histogram is a discrete function $h(l)$ that denotes the number of pixels with intensity $l$. Using this definition the (\ref{DIV2}) can be rewritten as  
\begin{equation}\label{DIV3}
\begin{split}
D_{AB}&=\sum_{l=0}^{L-1}[2 - (2 - \mu_A(l))e^{\mu_A(l) - 1} - \mu_A(l)e^{1- \mu_A(l)}  \\
&\qquad +2 - (2 - \mu_A(l) - \pi_A(l))e^{\mu_A(l) + \pi_A(l) - 1}\\
&\qquad - (\mu_A(l) + \pi_A(l)) e^{1 - \mu_A(l) - \pi_A(l)}]h(l)
\end{split}
\end{equation}
Since the membership function denotes the belongingness of a grey level to a region, then the smaller the difference between the grey level and the mean of the region to which the pixel belongs, the greater the membership value and vice versa. The membership function $\mu$ can be written as
\begin{equation}\label{membership}
\mu(x)=0.582(e^{1-|x-y|}-1)
\end{equation}
...
\begin{align}\label{membershiptwo}
\mu_A(l)&=0.582(e^{1-|l-m_0|}-1) \hspace{2 mm} \text{if} \; l \le t, \hspace{1 mm} \text{background}\\
	       &=0.582(e^{1-|l-m_1|}-1) \hspace{2 mm} \text{if} \; l > t,  \hspace{1 mm} \text{foreground}
\end{align}
where $m_0$ , $m_1$ are the average grey levels of the background and foreground (or object) region and may be written as
\begin{equation}\label{mean}
m_0=\frac{\sum_{l=0}^t l \; h(l)}{\sum_{l=0}^t h(l)}  \hspace{3 mm} m_1=\frac{\sum_{l=t+1}^{L-1} l \; h(l)}{\sum_{l=t+1}^{L-1} h(l)}
\end{equation}
According to this definition, the membership value is higher when the difference between the grey level and the mean of the region is smaller. In order to create IFSs from fuzzy set, intuitionistic fuzzy generators are used. In this work, intuitionistic fuzzy generator is created taking into account Sugeno generating function \cite{Sugeno}, according to which the hesitation degree function may be written as
\begin{equation}\label{hesitation}
\pi(l)=1-\mu(l) -\frac{1-\mu(l)}{1+\lambda\mu(l)}
\end{equation}
Under this interpretation of $\pi_A(x)$, if the expert is certain that a grey level belongs to the background or the object, then the hesitation degree must be zero. The value of $\pi_A(x)$ increases as the hesitance of the expert grows, that is when the membership degree is close to 0.5. However, the hesitance must have the least possible influence on the choice of the membership degree. Using the equation (\ref{hesitation}) the hesitation degree is related to the adopted $\lambda$ value. In \cite{Cha03} the $\lambda$ value was chosen during experiments, observing that with a too small $\lambda$ value some noise was still present in the thresholded images. So, a default value of $0.8$ was chosen and remained the same for each analysed image. In the same way here a default value has been taken in order to avoid further computation and observing that a too small $\lambda$ value is irrelevant for the computation of the divergence function, while a too big $\lambda$ value tends to increase too much the hesitation degree, moving the higher hesitance far from $\mu=0.5$, as it can be seen in Fig.~\ref{fplot}. For all these reasons, in the proposed approach $\lambda = 0.5$ has been chosen as a default value. Accordingly, in the worst case, the hesitance will have a maximum influence of 0.1. Finally, the divergence function is calculated for each possible threshold value, in the range $[L_{min}$ , $L_{max}]$, the minimum and maximum grey level value present in the image, respectively. Since the analysis is limited to the range $[L_{min}$ , $L_{max}]$ the computations of the equations (\ref{DIV3}) and (\ref{mean}) are optimized with summations that operate only in that range (summations that operate on all the grey levels in the histogram have been reported for clarity). The grey level corresponding to the minimum divergence value is selected as the optimal threshold value.
\begin{figure}[!bp]
\vspace{-5 mm}
\centering
%\includegraphics[width=0.9\textwidth]{images/plot}
\caption{\label{fplot}Influence of lambda value on the hesitation degree calculation.}
\end{figure}

\subsection{Local IFS threshold}
The proposed IFS threshold approach has showed excellent results if used on the whole image as a global threshold. Nevertheless images could be complex and can present histograms with more than two modes or an uneven illumination. So, local threshold (\ref{LT}) must be used in order to take into account local variations and multiple thresholds must be used in order to subdivide images into more meaningful regions. To combine these two approaches is not always simple and it can lead to a considerable increase of the computational cost. For all these reasons, in the proposed system, the original image has been divided into a prefixed number of equal sized sub-images. The number of sub-images chosen is 16, in order to deal with the peripheral windows differently from the central windows and to take into account the reduction of brightness or saturation at the periphery caused by camera settings or lens limitations (vignetting effect). Each sub-image is segmented separately and in order to consider also histograms with more than two modes a multiple threshold has been performed. The simplest way to realise a multiple threshold is to iterate a single threshold procedure until the number of needed threshold is reached. For example, if there are three regions in the sub-image, two threshold $t_1$ and $t_2$ must be selected, such that $L_{min} \le t_1 < t_2 \le L_{max}$. As showed previously, using an iterative single threshold procedure the first threshold can be easily obtained using the whole histogram from $L_{min}$  to $L_{max}$. The second threshold could be obtained using only the biggest part of the segmented histogram, from $L_{min}$  to $t_2$ or from $t_1$  to $L_{max}$. This approach is simple and computationally not expensive since for each iteration after the first it uses a smaller histogram. To determine how many threshold values must be selected for a single sub-image a preliminary histogram analysis has been introduced. In fact it must be noted that a sub-image could present only two modes histogram and thus a single threshold must be selected or even a single mode histogram, thus the threshold operation is no more required. An example of the sub-images variability is shown in Fig.\ref{fig:subimages}, where an original image has been divided into 16 sub-images of the same size and for each one the histogram has been computed.

\begin{figure}[!htbp]
\centering
%\includegraphics[width=0.45\textwidth]{images/subimages}
%\includegraphics[width=0.45\textwidth]{images/plot2}
\caption{\label{fig:subimages}An example of the sub-images variability and histogram differences.}
\end{figure}

As it can be noted some sub-images present a single mode histogram and many other present a two mode histogram but usually a typical blood image presents three modes identifying, respectively, white blood cells, red blood cells and plasma. Thus the histogram analysis is necessary to obtain further knowledges on the single sub-images, to be able to select appropriate threshold values and at the same time avoid iterations when they are not necessary. The histogram analysis performed is based on the average grey level (\ref{hmean}) and the standard deviation (\ref{hsd}). The standard deviation value is used to determine the modes number in the histogram, while the average value is used to choose the portion of the histogram for searching the second threshold. The standard deviation is firstly computed for the whole image in order to get a reference value $std_{tot}$ that is used to define a minimum standard deviation value that a sub-image should have when it presents three modes. This value, calculated as $0.6\cdot std_{tot}$, has been selected after many experimental results, showing strong accuracy in identifying three modes histogram. At this point the first threshold value is computed in the same way for all the sub-images and only those that present a standard deviation higher than the reference value are submitted to the second step of threshold selection. The average grey level value is now used to identify which portion of the histogram has already been thresholded. In fact, depending on the cells proportion in a sub-image and so the size of the single modes, the first threshold could be selected between the first two modes of the histogram, as it happens for the fifth and sixth sub-images of Fig.\ref{fig:subimages}, or between the last two modes of the histogram, as it happens for the second and third sub-images of Fig.\ref{fig:subimages}. Thus, if the threshold value already obtained is less or equal to the average grey level, it is assigned to $t_1$ and used as starting point (from $t_1$  to $L_{max}$) otherwise it is assigned to $t_2$ and used as ending point (from $L_{min}$  to $t_2$) for searching the second threshold value.

\subsection{Experimental evaluation}
The results of the proposed approach are firstly compared with two threshold approaches, that are Zack algorithms and with the fuzzy threshold method proposed in \cite{Cha03} in order to show how the use of the proposed local threshold approach can improve the segmentation accuracy. Fig.\ref{fig:results1} shows two original blood sample images and the results after thresholding with each technique. 
\begin{figure}[!htbp]
\vspace{-5 mm}
\centering
%\includegraphics[width=0.38\textwidth]{images/35rgb}
%\includegraphics[width=0.38\textwidth]{images/53rgb}\vspace{1 mm} 
%\includegraphics[width=0.38\textwidth]{images/35hist}
%\includegraphics[width=0.38\textwidth]{images/53hist}\vspace{1 mm} 
%\includegraphics[width=0.38\textwidth]{images/35zack}
%\includegraphics[width=0.38\textwidth]{images/53zack}\vspace{1 mm} 
%\includegraphics[width=0.38\textwidth]{images/35fuzzy}
%\includegraphics[width=0.38\textwidth]{images/53fuzzy}\vspace{1 mm} 
%\includegraphics[width=0.38\textwidth]{images/35t2}
%\includegraphics[width=0.38\textwidth]{images/53t2}
\caption{\label{fig:results1}From top to bottom: original RGB images, grey level histograms, segmentation results with Zack, fuzzy and the proposed IFS approaches.}
\end{figure}
Both the original images are affected by an uneven illumination, in particular they present shadows in the corners and a strong illumination in the centre, a typical effect of optical microscope. In particular, in the first case, it's possible to observe that with each threshold techniques the image centre is segmented quite accurately while the image corners are badly segmented, preserving the shadows presents in the original image. Instead, in the second case, it's possible to observe that none threshold technique is able to preserve all the blood cells showed in the original image, segmenting only white blood cells and adding noise in the whole image, especially in the image corners. The main difference compared to the first image is the high presence of white blood cells. In this case the histogram is very different from the previous one, as it can be seen in Fig. \ref{fig:results1} (the two histograms present a different number of modes). In both cases it is possible to note that using a single global threshold the segmentation result is not accurate, while using the local IFS threshold approach this problem is resolved and it is possible to segment correctly all the cells present in the images. Furthermore, as it can be seen in Fig.~\ref{fig:dataset} there are many differences between the images present in the ALL-IDB1, not only in terms of lighting but also in terms of resolution and magnification. For all these reasons in \cite{Put13b, Put13c} a subset of 33 images acquired from the same camera and under the same lighting conditions has been considered. With the approach proposed in this work it is possible to extend the experimentation to the whole dataset, being able to deal with local variations and with multiple modes. Some results of the whole process of multiple adaptive local threshold are showed in Fig.\ref{fig:finalresults1}. After thresholding operations, all the images have been cleaned using an area opening as showed previously (\ref{postp}). Then in order to highlight the results accuracy the contours of the segmented regions have been superimposed on the original images. As it can be seen with the proposed approach the segmentation results are very accurate, able to identify correctly each blood cell present in the image and performing a further segmentation level in order to detect the white blood cell nuclei. Finally, the ground-truth images previously described, have been compared with the segmentation result images in order to calculate the most common metrics for segmentation evaluation, that are: accuracy (\ref{accuracy}), sensitivity (\ref{TPR}), specificity (\ref{TNR}), FPR (\ref{FPR}), FNR (\ref{FNR}), precision (\ref{precision}) and F-measure (\ref{Fmeasure}). The proposed segmentation approach has been compared with the algorithms of Otsu (\ref{Otsu}), Zack (\ref{Zack}) and Chaira \cite{Cha10} properly modified in order to obtain two threshold values. Table~\ref{tab:table1} shows the average performances obtained with the ten tested samples. As it can be seen the most important values obtained with the proposed approach are higher for both the threshold values. 
  
\begin{figure}[t]
\centering
%\includegraphics[height=0.38\textwidth]{images/5w}\vspace{1 mm} 
%\includegraphics[height=0.38\textwidth]{images/47w}\vspace{1 mm} 
%\includegraphics[height=0.38\textwidth]{images/15w}\vspace{1 mm} 
%\includegraphics[height=0.38\textwidth]{images/53w}\vspace{1 mm} 
\caption{\label{fig:finalresults1}Original images superimposed with the contours of the segmented region with the first threshold (in green) and the second threshold (in red).}
\end{figure}
...
\begin{center}
\begin{tabular}{C{1.4cm}  C{1.0cm} C{1.0cm} C{1.0cm} C{1.0cm}C{1.0cm} C{1.0cm} C{1.4cm} C{1.4cm}}
   			&Otsu 1st th	&Otsu 2nd th	&Zack \hspace{1mm}1st th	&Zack 2nd th&Fuzzy 1st th&Fuzzy 2nd th&Proposed 1st th&	Proposed 2nd th	\\ \hline \hline
...
It must also be noted that the whole process of adaptive local threshold using the proposed equation is performed in less than a second and that a single global threshold could be performed on about 0.03 seconds (on average), so comparable with the crisp methods mentioned before. The results obtained show that this method is able to segment correctly the image components offering excellent computational performances. 
...
\begin{figure}[!htbp]
\centering
%\hspace{-1.1mm}\includegraphics[height=0.10\textheight]{images/3-1}\vspace{1mm}
%\includegraphics[height=0.10\textheight]{images/3-2}
%\includegraphics[height=0.10\textheight]{images/3-3}
%\includegraphics[height=0.10\textheight]{images/1-1}
%\includegraphics[height=0.10\textheight]{images/1-2}
%\includegraphics[height=0.10\textheight]{images/1-3}
%\includegraphics[height=0.10\textheight]{images/2-1}
%\includegraphics[height=0.10\textheight]{images/2-3}
%\includegraphics[height=0.10\textheight]{images/2-4}
%\includegraphics[height=0.10\textheight]{images/4-1}
%\includegraphics[height=0.10\textheight]{images/4-2}
%\includegraphics[height=0.10\textheight]{images/4-3}
...
\begin{center}
\centering
%\includegraphics[height=0.36\textwidth]{images/Im003_1}\vspace{1 mm}
%\includegraphics[height=0.36\textwidth]{images/Im003_1_label}
...
\begin{figure}[!b]
\centering
%\includegraphics[height=0.25\textwidth]{images/ROI1}
%\includegraphics[height=0.25\textwidth]{images/ROI2}
%\includegraphics[height=0.25\textwidth]{images/ROI3}
...
\begin{figure}[!htbp]
\centering
%\includegraphics[height=0.36\textwidth]{images/ROI4}
%\includegraphics[height=0.36\textwidth]{images/ROI5}
...
\begin{figure}[!htbp]
\centering
%\includegraphics[height=0.14\textheight]{images/crop-Fig11-1}
%\includegraphics[height=0.14\textheight]{images/crop-Fig11-2}
%\includegraphics[height=0.14\textheight]{images/crop-Fig11-3}
%\includegraphics[height=0.14\textheight]{images/crop-Fig11-4}
...
\begin{figure}[!htbp]
\centering
%\includegraphics[height=0.13\textheight]{images/crop-Fig12-2}
%\includegraphics[height=0.13\textheight]{images/crop-Fig12-3}
...
\centering
\begin{tabular}{cccc}
Image No  &    Manual count  &      Auto count      &    Accuracy\\ \hline \hline
Image1  &  		9                     &          	5		        &         55\%\\
Image2  &    	10                   &       		10		        &         100\%\\	
Image3  &		12                   &        	11		        &         91\%\\
Image4  &     	7                     &        	4		        &         57\%\\	
